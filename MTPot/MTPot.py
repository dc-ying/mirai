...
def main():
    ...
    if args.output:
        ...
        logging.basicConfig(
            ...
            format='%(asctime)s [%(name)s] %(levelname)s %(filename)s:%(lineno)s %(message)s')
    try:
        syslogger_handler = get_syslog_logger(config.syslog_address, config.syslog_port, config.syslog_protocol)
        honey_logger.info(
            "Setup syslog with parameters: IP:%s, PORT:%d, PROTOCOL:%s",
            config.syslog_address,
            config.syslog_port,
            config.syslog_protocol)
        honey_logger.addHandler(syslogger_handler)
